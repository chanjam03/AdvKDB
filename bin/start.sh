...
if $interactive; then
        ...
        START_RDB_CSV="${QEXEC} ${PROC_RDB_CSV} -p ${PORT_RDB_CSV} -tp ${PORT_TICK} -hdbp ${PORT_HDB} -libCommon ${LIB_COMMON} -libR ${LIB_R} -dirHDB ${DIR_HDB} </dev/null >> ${DIR_LOGS_PROC}/rdb1_csv.log 2>&1 &"
